...
export default function AlbumPlayer() {
  ...
  // Known-good double-buffer playback flow.
  ...
  // Additional preload-only buffers for the next few tracks.
  const preloadRefs = useRef<Array<HTMLAudioElement | null>>([]);
  ...
  useEffect(() => {
    for (let offset = 0; offset < PRELOAD_LOOKAHEAD; offset += 1) {
      const preloadEl = preloadRefs.current[offset];
      if (!preloadEl) {
        continue;
      }
      ...
      if (preloadEl.getAttribute("src") !== track.audioFile) {
        preloadEl.src = track.audioFile;
        preloadEl.load();
      }
    }
  }, [currentTrackIndex]);
  ...
  return (
    <div className="flex flex-col gap-10 w-full max-w-5xl mx-auto">
      ...
      {Array.from({ length: PRELOAD_LOOKAHEAD }, (_, index) => (
        <audio
          key={`preload-${index}`}
          ref={(element) => {
